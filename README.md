...
The checked-in `src/data/commands.json` was generated from `sf commands --json` using the Salesforce CLI (2.151.7): each entry keeps the command's official summary, help description, every non-global flag with its one-line help text, and all of its examples (762 across 267 commands). To refresh it with a current Salesforce CLI installation:
